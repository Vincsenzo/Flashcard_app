...
// Reset the elements and onclick button events:
function resetElementsInDOM() {
    articleElement = document.getElementById('article');
    markKnowElement = document.getElementById('numberOfKnown');
    markUnknowElement = document.getElementById('numberOfUnknown');
...
let reviewFlashcardsButton;
let addNewCardButton;
let termInput;
let definitionInput;
...
function renderCards(cards) {
    mainElement.innerHTML = `
        ...
        <article><input id="termInput" type="text" placeholder="term"><hr><input id="definitionInput" type="text" placeholder="definition"></article>
    `;
    mainElement.insertAdjacentHTML('beforeend', cards.map(card =>`
        <article> ${card.term} <br><hr> ${card.definition} </article>
    `).join(''));

    reviewFlashcardsButton = document.getElementById('reviewFlashcardsButton');
    addNewCardButton = document.getElementById('addNewCardButton');
    reviewFlashcardsButton.addEventListener("click", reviewFlashcardsHTML);
    addNewCardButton.addEventListener("click", addNewCard);
}

function listFlashcards(event, whichButton) {
    event.preventDefault();
    if (whichButton === 'all') {
        ...
        const filteredCards = flashcards.filter(card => {
            if (whichButton === 'know') return card.known;
            if (whichButton === 'dknow') return !card.known;
        })
        renderCards(filteredCards);
    }
}

function reviewFlashcardsHTML() {
    mainElement.innerHTML = originalContent;
    resetElementsInDOM();
}

// Sending new card data to server:
